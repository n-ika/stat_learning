    ...
    for stim_type in ['unigram','bigram','zerovec-bigram']:
        for model_type in ['onehot', 'phon', 'acoustic_new_norm']: 
            for exp in [1,2]:
                ...
                if model_type=='acoustic_new_norm' and stim_type=='unigram':
                    for N in [1,2,4,7,14]:
                        syll_vec = {k:s.reshape(s.shape[0], s.shape[1], 16, -1) for k,s in syll_vec.items()}
                        torch.save(syll_vec, out_dir+f'syll_vec_acoustic_vec_{N}_{stim_type}.pt')
                        train_in = train_in.reshape(-1, N, 16)
                        train_labels_in = np.repeat(train_labels_in, 28//N).tolist()
                        train_out = train_out.reshape(-1, N, 16)
                        train_labels_out = np.repeat(train_labels_out, 28//N).tolist()
                        test_in = test_in.reshape(-1, N, 16)
                        test_labels_in = np.repeat(test_labels_in, 28//N).tolist()
                        test_out = test_out.reshape(-1, N, 16)
                        ...
                        save_data('train_in',train_in,train_labels_in,train_conditions,out_dir,f'acoustic_vec_{N}')
                        save_data('train_out',train_out,train_labels_out,train_conditions,out_dir,f'acoustic_vec_{N}')
                        save_data('test_in',test_in,test_labels_in,test_conditions,out_dir,f'acoustic_vec_{N}')
                        save_data('test_out',test_out,test_labels_out, test_conditions, out_dir, f'acoustic_vec_{N}')
                        print(f'{stim_type} data, acoustic_vec_{N} model, exp: {exp}, train in shape: {train_in.shape}, train out shape: {train_out.shape}, test in shape: {test_in.shape}, test out shape: {test_out.shape}')
    print('data saved!')
