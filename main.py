...
def main(args):
    ...
    for model_type in ['onehot']: #, 'phon', 'acoustic_new_norm']:
        for stim_type in ['unigram']: #,'bigram','zerovec-bigram']: #FIXME
            ...
            for exp in [1,2]:
                ...
                if model_arch == 'AE':
                    root_dir = root+f'/ae_results_onehot/' #{loss_type}/' #FIXME
                elif model_arch == 'RNN':
                    root_dir = root+f'/rnn_results_{loss_type}/' #FIXME
                ...
                for simulation_num in range(1,num_simulations+1):
                    ...
                    out_name = (
                            f"res_{model_type}_{bottleneck_size}_exp-{exp}_n-{simulation_num}_lr-{lr}_train-{int(batch_size_train)}_test"
                    ...
                    model, loss_track, test_df = train_model(model,
                                                        optimizer,
                                                        loss_fun,
                                                        device,
                                                        epochs,
                                                        ...
                                                        batch_size_test,
                                                        model_ckpt_name)
                    test_df['stim_type'] = stim_type
                    test_df['model_type'] = model_type
                    test_df['lr'] = lr
                    test_df['exp'] = exp
                    test_df['model_num'] = simulation_num
                    test_df.to_csv(res_dir+f'/{out_name}.csv') 
                    pd.DataFrame(loss_track).to_csv(model_dir+f'/loss_{out_name}.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root', '-r', type=str, help='Root directory for data and results', default='/Users/nika/Desktop/hippocampus/stat_learning/zevin/modeling/')
    parser.add_argument('--model_arch', '-ma', type=str, required=True, choices=['AE', 'RNN'], default='AE', help='Model architectures to run')
    ...
    parser.add_argument('--num_simulations', '-ns', type=int, default=10, help='Number of simulations to run')
