...
def main(args):
    ...
    for stim_type in ['unigram','bigram','zerovec-bigram']:
        for model_type in ['onehot', 'phon', 'acoustic_new','acoustic_new_norm']: 
            for exp in [1,2]:
                if exp==1:
                    root_exp = modeling_root+'simulation_one/'
                    root_raw_stims = root + '/zevin_data/stimuli/simulation_one/'
                    test_items={'pa-bi-ku':1,'ti-bu-do':1,'tu-da-ro':0,'pi-go-la':0}
                    # train_items = {'pa-bi-ku': 1, 'ti-bu-do': 1, 'go-la-tu': 1, 'da-ro-pi': 1}
                elif exp==2:
                    root_exp = modeling_root+'simulation_two/'
                    root_raw_stims = root + '/zevin_data/stimuli/simulation_two/'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root', type=str, default='/Users/nika/Desktop/hippocampus/stat_learning/zevin/')
