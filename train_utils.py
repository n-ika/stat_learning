    ...
    for epoch in range(1,epochs+1):
        for i, in_batch_ in enumerate(in_loader):
            ...
            if batch_size_test==1:
                test_dfs.append(test_model(test_in_loader,test_out_list,model,device,i,loss_fun,epoch))
        if batch_size_test>1: 
            test_dfs.append(test_model(test_in_loader,test_out_list,model,device,0,loss_fun,epoch))
        print('done epoch & test #: ',epoch)                
        torch.save(model.state_dict(), f'{model_ckpt_name}_ep-{epoch}.pt')
    test_df = pd.concat(test_dfs)
    return(model, loss_track, test_df)